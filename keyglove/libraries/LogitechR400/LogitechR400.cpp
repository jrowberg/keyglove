...
//
// Dismantle R400 handheld, connect the Arduino SPI pins to the
// equivalent test points on the R400. Move any touch sensors that
// would normally be on those pins as per pins.h.
//
// WARNING: This connects to the first found R400 compatible reciever
...
void LogitechR400::init() {
    ...
    DEBUG_PRNL_R400("r400 SPI Init complete.");

    // Scan for a controller to connect to
    DEBUG_PRNL_R400("r400 Scanning for devices");
    bool found = false;
    for (uint16_t sop = 0; sop <= 10 && !found; sop++) {
        for (uint16_t i = 0; i <= 0x62 && !found; i++) {
            if (i == 0) {
                DEBUG_PRN_R400("r400 Scanning for SOP_CODE 0x");
                DEBUG_PRNLF_R400(sop, HEX);
                setSopCode(sop);
            }
            write(CHANNEL_ADR, i);
            write(XACT_CFG_ADR, 0xA7);
            if (!sendPacket(2, 0x41, 0x00)) {
                DEBUG_PRN_R400("r400 Found device on channel: 0x");
                DEBUG_PRNLF_R400(i, HEX);
                found = true;
            }
        } 
    }
    DEBUG_PRNL_R400("r400 Scan complete!");
}

bool LogitechR400::sendPacket(uint16_t count, ...) {
    ...
    va_start(ap, count);
    while (count){
        write(TX_BUFFER_ADR, va_arg(ap, uint16_t)); // Add the bytes to send
        count--;
    }
    va_end(ap);
...
void LogitechR400::setSopCode(uint16_t sopno) {
    switch (sopno) {
        case 0:
...
uint8_t LogitechR400::SPI(volatile uint8_t data) {
    long c = 0;
    SPDR = data;
...
uint8_t LogitechR400::read(uint8_t reg) {
    uint8_t res;
    CB(SPIPORT, SS);           
    SPI(READ|reg);
    res = SPI(0xFF);
...
void LogitechR400::write(uint8_t reg, uint8_t data) {
    CB(SPIPORT, SS);
    SPI(WRITE|reg);
    SPI(data);
    SB(SPIPORT, SS);  
...
void LogitechR400::set_modifier(uint8_t c) {
    keyboard_modifier_keys = c;
}

void LogitechR400::set_key1(uint8_t c) {
    keyboard_keys[0] = c;
}

void LogitechR400::set_key2(uint8_t c) {
    keyboard_keys[1] = c;
}

void LogitechR400::set_key3(uint8_t c) {
    keyboard_keys[2] = c;
}

void LogitechR400::set_key4(uint8_t c) {
    keyboard_keys[3] = c;
}

void LogitechR400::set_key5(uint8_t c) {
    keyboard_keys[4] = c;
}

void LogitechR400::set_key6(uint8_t c) {
    keyboard_keys[5] = c;
}

void LogitechR400::send_now() {
...
void LogitechR400::set_buttons(uint8_t left, uint8_t middle, uint8_t right) {
    uint8_t mask=0;
    if (left) mask |= 0x20;
    if (middle) mask |= 0x40;
    if (right) mask |= 0x80;
...
void LogitechR400::move(uint8_t x, uint8_t y) {
    move(x, y, 0);
}

void LogitechR400::move(uint8_t x, uint8_t y, uint8_t z) {
